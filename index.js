...
client.on("interactionCreate", interaction => {
  if (interaction.customId === "fast") {
    const queue = client.distube.getQueue(interaction);
       if (!queue) return interaction.reply(`Sem Musica na fila paizao.`)
    let data = db.fetch(`music_${interaction.guild.id}`)
    if (!data) return interaction.reply({content: "Num deu.Error **404**", ephemeral: true})
    queue.filters.add("nightcore")
interaction.reply({content: "Speed online!", ephemeral: true})
  }
  if (interaction.customId === "slowmode") {
    const queue = client.distube.getQueue(interaction);
       if (!queue) return interaction.reply(`Nao tem nada tocando :c.`)
    let data = db.fetch(`music_${interaction.guild.id}`)
